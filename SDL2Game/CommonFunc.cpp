    ...
    const std::string& img_name)
{
    ...
    text_object[0].LoadFromRenderText(font, gScreen);

    text_object[1].SetText(labels[1]);
    text_object[1].SetColor(color[0].r, color[0].g, color[0].b);
    text_object[1].LoadFromRenderText(font, gScreen);
    ...
    bool ret = gBackground.LoadImg(img_file, gScreen);
                ...
                for (int i = 0; i < kMenuNum; ++i)
                {
                        ...
                        y >= pos[i].y && y <= pos[i].y + pos[i].h)
                    {
                        if (!selected[i])
                        {
                            ...
                            text_object[i].LoadFromRenderText(font, gScreen);
                        }
                    }
                    else
                    {
